  ...
  const perSalesperson = useMemo(() => {
    ...
    return salespeople.map(sp => {
      ...
      return {
        ...
        openQuotes: openQuotes.length, awaiting: awaiting.length,
        activeDeals: activeDeals.length, avgDaysToDealNum,
        staleQuotes: staleQuotes.length,
        pendingMoves: pendingMv.length, overdueMoves: overdueMv.length,
        myQuotes, myMoves,
      };
    });
    ...
    <Shell>
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        <div className="flex justify-between items-center">
          ...
          <button onClick={loadAll} className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded text-sm">
            ↻ Refresh
          </button>
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {perSalesperson.map(sp => (
                    <RowWithDetail key={sp.id} sp={sp} isOpen={!!expanded[sp.id]} onToggle={() => setExpanded(e => ({ ...e, [sp.id]: !e[sp.id] }))} />
                  ))}
                </tbody>
              </table>
            </div>
        ...
        <section>
          ...
          {tab === 'deals' ? <DealETATable quotes={quotes} salespeople={salespeople} /> : <MoveETATable moves={moves} salespeople={salespeople} />}
        </section>
      </div>
    </Shell>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 text-sm">
            <span className="text-gray-400">
              {profile?.full_name}
              <span className="ml-2 px-2 py-0.5 bg-gray-800 rounded text-xs capitalize">{role?.replace('_',' ')}</span>
            </span>
...
function Card({ label, value, sub, tone = 'neutral' }) {
  const tones = { ok: 'border-gray-800', neutral: 'border-gray-800', warn: 'border-yellow-600/40', alert: 'border-red-600/50' };
  const subTones = { ok: 'text-gray-500', neutral: 'text-gray-500', warn: 'text-yellow-400', alert: 'text-red-400' };
  return (
  ...
  return (
    <>
      <tr className="border-t border-gray-800 hover:bg-gray-900/70 cursor-pointer" onClick={onToggle}>
        ...
        <td className="p-3 text-right text-gray-300">{sp.avgDaysToDealNum == null ? '—' : `${sp.avgDaysToDealNum.toFixed(1)}d`}</td>
        <td className="p-3 text-right"><AgingBadge count={sp.staleQuotes} zero="ok" /></td>
        <td className="p-3 text-right">{sp.pendingMoves}</td>
        <td className="p-3 text-right"><AgingBadge count={sp.overdueMoves} zero="ok" level="alert" /></td>
      </tr>
      {isOpen && (
        <tr className="bg-gray-900/40">
          <td></td>
          <td colSpan={8} className="p-3"><SalespersonDetail sp={sp} /></td>
        </tr>
      )}
    </>
  );
        ...
        {sp.myQuotes.filter(q => !q.deal_number).length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="text-gray-500 text-xs">
              <tr className="text-left">
                <th className="py-1">Quote #</th><th className="py-1">Customer</th><th className="py-1">Status</th><th className="py-1 text-right">Age</th>
              </tr>
            </thead>
            <tbody>
              {sp.myQuotes.filter(q => !q.deal_number).map(q => {
                ...
                return (
                  <tr key={q.id} className="border-t border-gray-800">
                    <td className="py-1"><Link to={`/quotes/${q.id}`} className="text-blue-400 hover:underline">{q.quote_number}</Link></td>
      ...
      <div>
        ...
        {sp.myMoves.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="text-gray-500 text-xs">
              <tr className="text-left">
                <th className="py-1">From → To</th><th className="py-1">Type</th><th className="py-1">Status</th><th className="py-1 text-right">Date Needed</th>
              </tr>
            </thead>
            <tbody>
              {sp.myMoves.map(m => (
...
function DealETATable({ quotes, salespeople }) {
  ...
  const now = Date.now();
  const rows = activeQuotes.map(q => ({ ...q, age: daysSince(q.created_at, now), sp: salespeople.find(s => s.id === q.salesperson_id)?.full_name ?? '—' })).sort((a,b) => b.age - a.age);
    ...
    <div className="overflow-x-auto rounded-lg border border-gray-800">
      <table className="w-full text-sm">
        <thead className="bg-gray-900 text-gray-400 text-xs">
          <tr className="text-left">
            <th className="p-3">Quote #</th><th className="p-3">Salesperson</th><th className="p-3">Customer</th><th className="p-3">Deal #</th><th className="p-3">Status</th><th className="p-3 text-right">Age</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(q => (
            <tr key={q.id} className="border-t border-gray-800 hover:bg-gray-900/70">
              <td className="p-3"><Link to={`/quotes/${q.id}`} className="text-blue-400 hover:underline">{q.quote_number}</Link></td>
...
function MoveETATable({ moves, salespeople }) {
  const rows = moves.map(m => ({ ...m, sp: salespeople.find(s => s.id === m.requester_id)?.full_name ?? '—' })).sort((a,b) => {
    const aOv = isOverdue(a); const bOv = isOverdue(b);
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-800">
      <table className="w-full text-sm">
        <thead className="bg-gray-900 text-gray-400 text-xs">
          <tr className="text-left">
            <th className="p-3">From → To</th><th className="p-3">Requester</th><th className="p-3">Type</th><th className="p-3">Region</th><th className="p-3">Status</th><th className="p-3 text-right">Date Needed</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(m => (
...
function TabBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick} className={`px-4 py-2 font-semibold border-b-2 transition-colors ${active ? 'text-orange-400 border-orange-500' : 'text-gray-400 border-transparent hover:text-white'}`}>
      {children}
    </button>
  );
}
...
function AgingBadge({ count, level = 'warn', zero = 'dim' }) {
  if (count === 0) return <span className="text-gray-600">0</span>;
...
function DateNeededLabel({ date }) {
  ...
  let cls = 'text-green-400'; let label = d.toLocaleDateString();
